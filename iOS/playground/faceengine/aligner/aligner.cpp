...
private:
	...
	float* GetSrcPoints(const std::vector<cv::Point2f>& keypoints) {
...
int Aligner::Impl::AlignFace(const cv::Mat & img_src,
	...
	cv::Mat src_mat(5, 2, CV_32FC1, &points_src[0]);
...
int Aligner::Impl::MatrixRank(const cv::Mat& M) {
	cv::Mat w, u, vt;
	cv::SVD::compute(M.clone(), w, u, vt);
