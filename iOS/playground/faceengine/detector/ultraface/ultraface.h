...
namespace mirror {
class UltraFace : public Detector {
public:
	UltraFace();
	~UltraFace();
	int Init(const char* model_path);
	int InitMem(void* buffer, size_t size, const int threads);
	int DetectFace(const cv::Mat& img_src, std::vector<FaceInfo>* faces, const float scoreThreshold);
