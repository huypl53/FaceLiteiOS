...
int Centerface::DetectFace(const cv::Mat & img_src, std::vector<FaceInfo>* faces, const float scoreThreshold) {
	...
	// get output
	auto tensor_heatmap = centerface_interpreter_->getSessionOutput(centerface_sess_, "537");
	auto tensor_scale = centerface_interpreter_->getSessionOutput(centerface_sess_, "538");
	auto tensor_offset = centerface_interpreter_->getSessionOutput(centerface_sess_, "539");
	auto tensor_landmark = centerface_interpreter_->getSessionOutput(centerface_sess_, "540");
	...
	for (int h = 0; h < output_height; ++h) {
		for (int w = 0; w < output_width; ++w) {
			...
			cv::Rect face = cv::Rect(scale_x * xmin, scale_y * ymin, scale_x * (xmax - xmin), scale_y * (ymax - ymin));
			...
			face_info.location_.x = center_x - 0.5f * max_side;
			face_info.location_.y = center_y - 0.5f * max_side;
