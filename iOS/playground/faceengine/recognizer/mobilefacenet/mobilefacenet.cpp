...
int Mobilefacenet::InitMem(void *buffer, size_t size) {
    mobilefacenet_interpreter_ = std::unique_ptr<MNN::Interpreter>(MNN::Interpreter::createFromBuffer(buffer, size));
    if (nullptr == mobilefacenet_interpreter_) {
        std::cout << "load model failed." << std::endl;
        return 10000;
    }
